...
|-----|----------|---------|
| **Host errors** | `logs/calcifer.error.log` | Delivery failures, crash-loop backoff, warnings — check this first |
| **Host app log** | `logs/calcifer.log` | Full routing chain: inbound routing, container spawn/exit, delivery |
...
Reading the flow:
- `messages_in` has the message but no matching `messages_out` → the container never produced a reply (check `processing_ack`, then `logs/calcifer.log` for spawn/exit and container stderr).
...
A spawned container that exits without writing to `outbound.db` shows up in `logs/calcifer.log` as a `Container exited` line with a non-zero `code`, often preceded by streamed `container=<folder>` stderr (at debug level).
...
Session and group folders are bind-mounted into the container. To see the resolved mounts for a spawn, run with `LOG_LEVEL=debug` and read the spawn command in `logs/calcifer.log`, or grep the mount targets directly:
...
tail -n 5 logs/calcifer.error.log 2>/dev/null || echo "No error log yet"
```
