...
---
▸ 2026-08-17T18:42:40Z
VERIFIED + DONE. Owner tested the shared-link-through-login flow on the live public domain: opened a /app/<view>/<id> deep link in an incognito window as a second user (web:alicia), hit the login gate with the path preserved, signed in, and landed directly on the record. Confirms the SPA history fallback + client-side path-preserving auth work end-to-end for a shared link, incl. multi-user auth against family-shared data. No code change was needed.
