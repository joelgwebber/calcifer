...
function insertMessage(
  ...
  kind: string,
  content: object,
  opts?: { timestamp?: string; trigger?: number; processAfter?: string },
) {
  const timestamp = opts?.timestamp ?? new Date().toISOString();
  const trigger = opts?.trigger ?? 1;
  getInboundDb()
    .prepare(
      `INSERT INTO messages_in (id, kind, timestamp, status, trigger, process_after, content, seq)
       VALUES (?, ?, ?, 'pending', ?, ?, ?, ?)`,
    )
    .run(id, kind, timestamp, trigger, opts?.processAfter ?? null, JSON.stringify(content), nextSeq++);
}
