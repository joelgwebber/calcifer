...
# Verify yaks vendor is present (it's baked into the built image) — run
# sync-yaks.sh to update it. Only relevant when actually building; a pulled
# image ships its own copy.
if [ "$PULL" != "true" ] && [ ! -f "tools/yaks/yak.py" ]; then
