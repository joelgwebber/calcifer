...
export function createChannelDeliveryAdapter(): ChannelDeliveryAdapter {
  return {
    ...
    // Ephemeral rich activity label pushed to the web UI over SSE
    // (calcifer-5b6b.2). Only the web adapter implements setStatus; coarse
    // channels omit it and fall back to setTyping. Exact-key resolution,
    // same discipline as deliver/typing dispatch.
    async setStatus(
      channelType: string,
      platformId: string,
      threadId: string | null,
