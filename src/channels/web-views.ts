...
async function resolveAgentGroupFolder(userId: string): Promise<string | null> {
  const mg = await getMessagingGroupByPlatform('web', userId);
  if (!mg) return null;
  const agents = await getMessagingGroupAgents(mg.id);
  if (agents.length === 0) return null;
  const ag = await getAgentGroup(agents[0].agent_group_id);
...
export async function fileForUser(userId: string, viewName: string, id: string): Promise<ViewFile | null> {
  const manifest = getView(viewName);
  if (!manifest) throw new ViewDataError(404, `unknown view: ${viewName}`);
  const folder = await resolveAgentGroupFolder(userId);
  if (!folder) throw new ViewDataError(404, 'no agent group for this user');
 ...
 */
export async function resolveRecordCard(viewName: string, id: string): Promise<WebCard | null> {
  const manifest = getView(viewName);
  if (!manifest) return null;
  let record: Record<string, unknown> | null;
  try {
    record = await getViewRecord(manifest, '', id);
  } catch {
    return null;
  }
 ...
 */
export async function cardFromContent(content: unknown): Promise<WebCard | null> {
  if (content && typeof content === 'object') {
    const c = content as Record<string, unknown>;
    if (c.type === 'record_card' && typeof c.view === 'string' && typeof c.id === 'string') {
      const resolved = await resolveRecordCard(c.view, c.id);
...
export async function annotateForUser(
  viewName: string,
  entityId: string,
  key: string,
  value: string | null,
  ...
  if (value === null) await clearAnnotation(manifest.skill, entityId, key);
  else await setAnnotation(manifest.skill, entityId, key, value);
}
