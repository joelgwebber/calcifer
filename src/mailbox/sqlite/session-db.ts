...
/**
 * Read the ephemeral mid-turn activity label (calcifer-5b6b.2) the container
 * wrote to session_state. Host delivery reads it and pushes it to the web
 * client over SSE via setStatus. `null` means no label is set (turn idle or
 * cleared). Best-effort: an older session DB without the row returns null.
 */
export function getActivityStatus(outDb: Database.Database): string | null {
  try {
